...
public struct CHMarkdownView<Content: View>: View {
    ...
    var transformedNodes: [IndexASTNode] {
        ...
        if let cache = cache, let cachedNodes = cache.get(for: text) {
            ...
            let parser = CHParser(text)
            cache?.set(parser.nodes, forText: text)
            nodes = parser.nodes
        }
        return CHSwiftUIMapper.transform(nodes: nodes).enumerated().map { index, node in
    ...
    public func useCache(cache: CHMarkdownParserCache = .shared) -> some View {
